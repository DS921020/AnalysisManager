    ...
    $.post('/DataModel/GetMenu',data,function (result){
        if(result.result.status=='200'){
            ...
            for(var i=0;i<menudata.length;i++){
                 firstlevelli+='<li class="lis">';
                 firstlevelli+=' <a>';
                 firstlevelli+=' <i class="fl glyphicon glyphicon-home"></i>';
                 firstlevelli+=' <span class="fl">'+menudata[i].name+'</span>';
                 firstlevelli+=' <i class="fr guodu glyphicon glyphicon-triangle-bottom"></i>';
                 firstlevelli+=' </a>';
                 firstlevelli+=' <ul class="lis-out">';
                 var secondlevelli='';
                 for(var j=0;j<menudata[i].content.length;j++){
                     secondlevelli+=' <li class="lis">';
                     secondlevelli+=' <a class="lis-a" ';
                     secondlevelli+=' data-id=';
                     secondlevelli+=menudata[i].content[j].id;
                     secondlevelli+=' data-href=';
                     secondlevelli+=menudata[i].content[j].url;
                     secondlevelli+=' >';
                     secondlevelli+=' <span class="fl">';
                     secondlevelli+=menudata[i].content[j].name;
                     secondlevelli+=' </span>';
                     secondlevelli+=' </a>';
                     secondlevelli+='  </li>';
                 }
                 firstlevelli+=secondlevelli;
                 firstlevelli+='</ul>';
                 firstlevelli+='</li>';
             }

            $('.navbar-scroll-inner').append(firstlevelli);
            // 菜单动效
            ...
            $.ajax({
                type : "GET",
                url : "/static/json/index_setting.json",
                data : "json",
            ...
            $(".navbar").on("click",".lis-a",function(e){
                ...
                for (var i = 0; i < $('.page-tabs-content').length; i++) {
                    //console.log($('.page-tabs-content').eq(i).attr('data-id'));
